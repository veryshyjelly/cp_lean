...
def zeroMatrix (n : Nat) : Array (Array Int) :=
  Array.replicate n (Array.replicate n 0)

def setAt (m : Array (Array Int)) (i j : Nat) (v : Nat) :=
  if h₁ : i < m.size then
    let row := m[i]
...
def fillMatrix
  ...
  (lastFill : Nat)
  (lastLoc : Nat × Nat)
  (arr : Array (Array Int)) : Array (Array Int) :=
...
def solution : List (List Int) → ListOf (ListOf Int)
| [[n]] =>
  let n := n.toNat
  let firstC := (n - 1) / 2
